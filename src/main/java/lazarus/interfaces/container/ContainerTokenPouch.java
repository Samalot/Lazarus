...
import lazarus.interfaces.container.slots.SlotArmour;
import lazarus.interfaces.container.slots.SlotTokenPouch;
...
/*Main*/
public class ContainerTokenPouch extends Container
{
	/** The Item Inventory for this Container */
	private final InventoryTokenPouch inventory;

	private static final int
	ARMOR_START = InventoryTokenPouch.INV_SIZE, // INV_SIZE = 10, so slots 0 to 9 are the custom inventory, armor starts at the next slot (i.e. 10)
	...
	public ContainerTokenPouch(EntityPlayer player, InventoryPlayer inv, InventoryTokenPouch bag)
	{
		int i = 0;
		inventory = bag;

		// CUSTOM INVENTORY SLOTS
		for (i = 0; i < InventoryTokenPouch.INV_SIZE; ++i) {
			addSlotToContainer(new SlotTokenPouch(inventory, i, 80 + (18*(i%5)), 8 + (18 * (int)(i/5))));
		}

		// ARMOR SLOTS
		for (i = 0; i < 4; ++i) {
			addSlotToContainer(new SlotArmour(player, inv, inv.getSizeInventory() - 1 - i, 8, 8 + i * 18, i));
		}
